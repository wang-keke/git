...
public class ArticleDaoImpl implements ArticleDao {
    ...
    public boolean insertArticle(Article a) {
        ...
        String sql = "insert into article(article_content,article_likenum,article_views,article_image,article_title,article_createtime,article_isTop,category_id,article_desc,count_comment)"
                + " values(?,?,?,?,?,?,?,?,?,0)";
    ...
    public boolean updateArticle(Article a) {
        ...
        Connection conn = DBUtil.getConnection();
        String sql = "update article set article_content = ?,article_image = ?,article_title = ?,article_createtime = ?,article_isTop = ?,category_id = ?,article_desc = ?  where article_id=?";
        PreparedStatement prep;
        try {
            prep = conn.prepareStatement(sql);
            prep.setString(1, a.getArticle_content());
    ...
    public ArrayList<Article> getPageArticle(int page, int limit) {
        ...
        // 预编译
        try {
            String sql = "select * from article limit " + startRow + "," + limit + "";
            PreparedStatement prep = conn.prepareStatement(sql);
            ResultSet rst = prep.executeQuery();
            while (rst.next()) {
                // 一共有九个值
    ...
    public Article getTopArticle(int article_isTop) {
        ...
        Connection conn = DBUtil.getConnection();
        // 在数据库中修改记录
        String sql = "select * from article where article_isTop = ?";
        // 预编译
        PreparedStatement prep;
        try {
            prep = conn.prepareStatement(sql);
    ...
    public ArrayList<Article> getSearchArticle(String article_title) {
        ...
        Connection conn = DBUtil.getConnection();
        // 在数据库中添加记录
        String sql = "select * from article where article_title like '%" + article_title + "%'";
        // 预编译
        try {
            PreparedStatement prep = conn.prepareStatement(sql);
            ResultSet rst = prep.executeQuery();
    ...
    public int getArticleId(long article_createtime) {
        ...
        int id = 0;
        // 在数据库中添加记录
        String sql = "select * from article where article_createtime = ?";
        try {
            PreparedStatement prep = conn.prepareStatement(sql);
    ...
    public ArrayList<Article> getSearchPageArticle(String article_title, int page, int limit) {
        ...
        int startRow = cp.getStartRow();
        // 在数据库中添加记录
        String sql = "select * from article where article_title like '%" + article_title + "%' limit " + startRow
                + "," + limit + "";
        // 预编译
        try {
            PreparedStatement prep = conn.prepareStatement(sql);
    ...
    public ArrayList<Article> getCategoryArticle(int category_id, int page, int limit) {
        ...
        int startRow = cp.getStartRow();
        // 在数据库中添加记录
        String sql = "select * from article where category_id = ? limit ?,?";
        // 预编译
        try {
    ...
    public ArrayList<Article> getRandomArticle(int limit) {
        ...
        // 在数据库中添加记录
        String sql = "SELECT * FROM article " +
                "WHERE article_id >= (SELECT floor(RAND() * (SELECT MAX(article_id) FROM article))) " +
    ...
    public ArrayList<Article> getHotArticle(int limit) {
        ...
        Connection conn = DBUtil.getConnection();
        // 在数据库中添加记录
        String sql = "SELECT * FROM article ORDER BY article_likenum desc limit ?";
        // 预编译
        try {
            aList = new ArrayList<Article>();
            PreparedStatement prep = conn.prepareStatement(sql);
    ...
    public ArrayList<Article> getNearArticle(int limit) {
        ...
        Connection conn = DBUtil.getConnection();
        // 在数据库中添加记录
        String sql = "SELECT * FROM article ORDER BY article_createtime desc limit ?";
        // 预编译
        try {
            aList = new ArrayList<Article>();
            PreparedStatement prep = conn.prepareStatement(sql);
